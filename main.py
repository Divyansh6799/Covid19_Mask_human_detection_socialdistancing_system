...
while (cap.isOpened()):
    ...
    image = cv2.resize(image, (1500, 780))
    ...
    for i in nsd:
        ...
        text = "Alert"
        cv2.putText(image, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
        cv2.putText(image, "Maintain Social Distancing", (12,42), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > confidence_threshold:
            ...
            label = "Mask Detected" if mask > without_mask else "No Mask Detected"
            color = (0, 255, 0) if label == "Mask" else (0, 0, 255)

            label = "{}: {:.2f}%: ID={}".format(label, max(mask, without_mask) * 100,i+1)
            c = cv2.putText(image, label, (startX+70, startY - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.70, color, 2)
    ...
    cv2.imshow("Mask Detection And Social Distance",image)
    # cv2.imshow("Face", x)
